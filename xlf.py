...
class Xlf():
    ...
    def get_segment(self):
        ...
        for file in self.root.findall('xliff:file', ns):
            file_obj = File(file.get('original'))
            for trans_unit_element in file.findall('xliff:body/xliff:trans-unit', ns):
                trans_unit_obj = TransUnit(trans_unit_element.get('id'))

                trans_unit_obj.source = create_seg_obj(trans_unit_element, "source", ns)
                trans_unit_obj.seg_source = create_seg_obj(trans_unit_element, "seg-source", ns)
                trans_unit_obj.seg_target = create_seg_obj(trans_unit_element, "target", ns)

                file_obj.segments.append(trans_unit_obj)
            files.append(file_obj)
        return files

def create_seg_obj(trans_unit_element, tag, ns):
    element = trans_unit_element.find('xliff:'+tag, ns)
    ...
    if (tag == "source"):
        ...
        mrks = []
        for mrk_element in element.findall('xliff:mrk', ns):
